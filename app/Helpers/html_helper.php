...
if (!function_exists('select2')) {
	function select2($elements = [], $id = "", $name = "", $value = [], $title = "Digite uma Opção", $multiple = false, $class = "form-control", $action = "")
	{
		$str = "<select id=\"$id\" name=\"$name\" " . (stripos($class, "required") ? 'required' : '') . " title=\"$title\" class=\"input-select2 \" " . ($multiple ? 'multiple' : '') . "  $action><option value=\"\">$title</option>";
		foreach ($elements as $key => $val) {
			$keys = array_keys($val);
			$str .= "<option " . (in_array($val[$keys[0]], $value) ? 'selected' : '') . " value=\"" . $val[$keys[0]] . "\">" . $val[$keys[1]] . "</option>";
		}
